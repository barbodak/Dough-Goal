...
inline int heat_color(const double &x) {
    // return std::floor( ( std::cos( x ) + 1.0 ) * HEAT_COLOR_SCALE );
    return static_cast<int>(std::floor((std::cos(x) + 1.0) * HEAT_COLOR_SCALE));
}

inline void debug_paint_evaluate_color(const Vector2D &pos, const double &value,
...
ActionChainGraph::ActionChainGraph(const FieldEvaluator::ConstPtr &evaluator,
    ...
    : M_evaluator(evaluator), M_action_generator(generator), M_chain_count(0),
      M_best_chain_count(0), M_max_chain_length(max_chain_length),
      M_max_evaluate_limit(max_evaluate_limit), M_result(),
...
void ActionChainGraph::calculateResult(const WorldModel &wm) {
    ...
    if (M_result.empty()) {
        ...
        CooperativeAction::Ptr action(
            new HoldBall(wm.self().unum(), wm.ball().pos(), 1, "defaultHold"));
    ...
    if (first_layer->action().category() == CooperativeAction::Pass)
        OffensiveDataExtractor::i().generate_save_data(wm,
                                                       first_layer->action());
...
#ifdef DEBUG_PROFILE
    dlog.addText(Logger::ACTION_CHAIN,
                 __FILE__ ": PROFILE size=%d elapsed %f [ms]", M_chain_count,
                 msec);
#endif
#ifdef ACTION_CHAIN_LOAD_DEBUG
    std::fprintf(
        ...
        wm.self().unum(), wm.time().cycle(), msec, n_evaluated,
    ...
    if (!S_evaluated_points.empty()) {
        ...
        const std::vector<std::pair<Vector2D, double>>::const_iterator end =
            S_evaluated_points.end();
        for (std::vector<std::pair<Vector2D, double>>::const_iterator it =
                 S_evaluated_points.begin();
             it != end; ++it) {
            if (min_eval > it->second)
                min_eval = it->second;
            if (max_eval < it->second)
                max_eval = it->second;
        }
        ...
        for (std::vector<std::pair<Vector2D, double>>::const_iterator it =
                 S_evaluated_points.begin();
             it != end; ++it) {
            debug_paint_evaluate_color(it->first, it->second, min_eval,
                                       max_eval);
        }
    }
#endif
 ...
 */
bool ActionChainGraph::doSearch(const WorldModel &wm, const PredictState &state,
...
#endif
#ifdef DEBUG_PAINT_EVALUATED_POINTS
    S_evaluated_points.push_back(
        std::pair<Vector2D, double>(state.ball().pos(), max_ev));
#endif
    ++(*n_evaluated);
    ...
    for (std::vector<ActionStatePair>::const_iterator it = candidates.begin();
        ...
        double ev;
        const bool exist =
            doSearch(wm, (*it).state(), new_path, &candidate_result, &ev,
                     n_evaluated, max_chain_length, max_evaluate_limit);
        if (!exist) {
            continue;
        }
#ifdef ACTION_CHAIN_DEBUG
...
#ifdef DEBUG_PAINT_EVALUATED_POINTS
        S_evaluated_points.push_back(
            std::pair<Vector2D, double>(it->state().ball().pos(), ev));
#endif
        if (ev > max_ev) {
            max_ev = ev;
...
double ActionChainGraph::calcDangerEvalForTarget(const WorldModel &wm,
                                                 Vector2D target) {
...
double ActionChainGraph::calcDangerEvalForBhv(const WorldModel &wm,
                                              const ActionStatePair &bhv) {
    Vector2D bhv_target = bhv.M_action->targetPoint();
    return calcDangerEvalForTarget(wm, bhv_target);
}
double
ActionChainGraph::calcDangerEvalForChain(const WorldModel &wm,
 ...
 */
void ActionChainGraph::calculateResultBestFirstSearch(
    const WorldModel &wm, unsigned long *n_evaluated)

{
    ...
    *(n_evaluated) = 0;
    ...
    double danger_eval =
        calcDangerEvalForTarget(wm, current_state.ball().pos());
    ...
    for (;;) {
        ...
        if (series.size() < M_max_chain_length &&
            ...
            dlog.addText(
                ...
                (series.empty() ? "empty"
                                : series.rbegin()->action().description()),
...
#ifdef ACTION_CHAIN_DEBUG
                dlog.addText(Logger::ACTION_CHAIN, "<<<< update best result.");
#endif
...
void ActionChainGraph::debugPrintCurrentState(const WorldModel &wm) {
    ...
    for (int n = 1; n <= ServerParam::i().maxPlayer(); ++n) {
        if (n == wm.self().unum()) {
            dlog.addText(Logger::ACTION_CHAIN, "n = %d, self", n);
        } else {
            const AbstractPlayerObject *t = wm.teammate(n);

            if (t) {
                dlog.addText(Logger::ACTION_CHAIN, "n = %d, OK", n);
            } else {
                dlog.addText(Logger::ACTION_CHAIN, "n = %d, null", n);
            }
        }
    }
    dlog.addText(Logger::ACTION_CHAIN, "=====");
    ...
    for (AbstractPlayerCont::const_iterator it = wm.allTeammates().begin();
         it != end; ++it) {
        if ((*it)->isSelf()) {
            dlog.addText(Logger::ACTION_CHAIN, "teammate %d, self",
                         (*it)->unum());
        } else {
            dlog.addText(Logger::ACTION_CHAIN, "teammate %d", (*it)->unum());
        }
    }
 ...
 */
void ActionChainGraph::debug_send_chain(
    PlayerAgent *agent, const std::vector<ActionStatePair> &path) {
    ...
    for (size_t i = 0; i < path.size(); ++i) {
        ...
        case CooperativeAction::Pass: {
            ...
            if (i == 0 &&
                ...
                if (s0->ourPlayer(action.targetPlayerUnum())
                        ->pos()
                        .dist(s1->ball().pos()) > DIRECT_PASS_DIST) {
 ...
 */
void ActionChainGraph::write_chain_log(const WorldModel &world, const int count,
                                       const std::vector<ActionStatePair> &path,
                                       const double &eval) {
...
void ActionChainGraph::write_chain_log(const std::string &pre_log_message,
    ...
    for (size_t i = 0; i < path.size(); ++i) {
        ...
        case CooperativeAction::Dribble: {
            dlog.addText(
                Logger::ACTION_CHAIN,
                "__ %d: dribble (%s[%d]) t=%d unum=%d target=(%.2f %.2f)", i,
                a.description(), a.index(), s1->spendTime(),
