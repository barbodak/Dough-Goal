...
OffensiveDataExtractor::OffensiveDataExtractor() : last_update_cycle(-1) {}
...
void OffensiveDataExtractor::init_file(DEState &state) {
    ...
    std::string rand_name = "im doing random shit";
    str += "_" + std::to_string(state.wm().self().unum()) + "_" +
           state.wm().theirTeamName() + "_E" + rand_name + ".csv";
...
std::string OffensiveDataExtractor::get_header() {
    ...
    for (int i = 1; i <= 11; i++) {
        ...
        if (option.openAnglePass == TM || option.openAnglePass == BOTH) {
            ...
            header +=
                "p_l_" + std::to_string(i) + "_pass_opp_dist_proj_to_opp,";
            header +=
                "p_l_" + std::to_string(i) + "_pass_opp_dist_proj_to_kicker,";
...
void OffensiveDataExtractor::generate_save_data(const WorldModel &wm,
    ...
    if (!update_shoot) {
        if (action.category() > 2 || !action.targetPoint().isValid() ||
            action.targetPlayerUnum() > 11 || action.targetPlayerUnum() < 1)
            return;
    }
    ...
    // output
    if (!update_shoot) {
        extract_output(state, action.category(), action.targetPoint(),
                       action.targetPlayerUnum(), action.description(),
...
void OffensiveDataExtractor::extract_players(DEState &state) {
    ...
    for (uint i = 0; i < players.size(); i++) {
        ...
        if (player == nullptr) {
            add_null_player(invalid_data_, (i <= 10 ? TM : OPP));
#ifdef ODEDebug
...
#ifdef ODEDebug
        dlog.addText(Logger::BLOCK, "## start extracting for p side%d unum%d",
                     player->side(), player->unum());
#endif
...
std::vector<DEPlayer *> OffensiveDataExtractor::sort_players(DEState &state) {
    ...
    for (int i = 1; i <= 11; i++) {
        ...
        if (player == nullptr || player->unum() < 0 ||
            !player->pos().isValid() || player->isGhost()) {
...
void OffensiveDataExtractor::extract_output(DEState &state, int category,
                                            const rcsc::Vector2D &target,
                                            const int &unum, const char *desc,
                                            double ball_speed) {
...
void OffensiveDataExtractor::extract_pass_angle(DEPlayer *player,
                                                DEState &state,
                                                ODEDataSide side) {
...
#ifdef ODEDebug
            dlog.addText(Logger::BLOCK,
                         "#### add invalid data for open angle pass");
#endif
...
#ifdef ODEDebug
            dlog.addText(Logger::BLOCK,
                         "#### add invalid for nearest opp dist");
#endif
    ...
    for (const auto &opp : state.opponents()) {
        ...
        opp_dist_angle.push_back(std::make_pair(
            opp->pos().dist(tm_pos), (opp->pos() - tm_pos).th().degree()));
        AngleDeg diff = (tm_pos - ball_pos).th() - (opp->pos() - ball_pos).th();
#ifdef ODEDebug
        dlog.addText(Logger::BLOCK,
                     "######check opp %d in %.1f,%.1f, diff:%.1f", opp->unum(),
                     opp->pos().x, opp->pos().y, diff.degree());
#endif
        if (diff.abs() > 60)
            continue;
    ...
    if (option.openAnglePass == side || option.openAnglePass == BOTH) {
        auto open_angle_sorter = [](ODEOpenAngle &p1,
                                    ODEOpenAngle &p2) -> bool {
...
#ifdef ODEDebug
            dlog.addText(Logger::BLOCK, "###### add opp %d angle pass first",
                         candidates[0].unum);
#endif
            ADD_ELEM("pass_opp_dist",
                     convertor_dist(candidates[0].dist_self_to_opp));
            ADD_ELEM("pass_opp_dist_proj_to_opp",
                     convertor_dist(candidates[0].dist_opp_proj));
            ADD_ELEM("pass_opp_dist_proj_to_kicker",
                     convertor_dist(candidates[0].dist_self_to_opp_proj));
            ADD_ELEM("pass_opp_open_angle",
                     convertor_angle(candidates[0].open_angle));
        } else {
#ifdef ODEDebug
            dlog.addText(Logger::BLOCK,
                         "###### add opp angle pass first invalid");
#endif
            ADD_ELEM("pass_opp_dist", invalid_data_);
            ADD_ELEM("pass_opp_dist_proj_to_opp", invalid_data_);
            ADD_ELEM("pass_opp_dist_proj_to_kicker", invalid_data_);
...
#ifdef ODEDebug
            dlog.addText(Logger::BLOCK,
                         "###### add opp pass dist first invalid");
#endif
            ADD_ELEM("opp_dist", invalid_data_);
            ADD_ELEM("opp_angle", invalid_data_);
        }
...
void OffensiveDataExtractor::extract_pos(DEPlayer *player, DEState &state,
                                         ODEDataSide side) {
...
void OffensiveDataExtractor::extract_base_data(DEPlayer *player,
                                               ODEDataSide side,
                                               DEState &state) {
    if (option.unum == side || option.unum == BOTH) {
        if (player->unum() == -1) {
...
uint OffensiveDataExtractor::find_unum_index(DEState &state, uint unum) {
    auto players = sort_players(state);
    if (players.size() < 11)
        std::cout << state.kicker()->unum() << " "
                  << "size problems" << players.size() << std::endl;
    for (uint i = 0; i < 11; i++) {
    ...
    std::cout << state.kicker()->unum() << " "
              << "not match" << players.size() << std::endl;
    return 0;
}
