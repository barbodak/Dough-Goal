...
enum ODEDataSide { NONE, TM, OPP, BOTH, Kicker };
...
class OffensiveDataExtractor {
  ...
  public:
    ...
    void extract_output(DEState &state, int category,
                        const rcsc::Vector2D &target, const int &unum,
...
class ODEOpenAngle {
  public:
    ...
    ODEOpenAngle(){};
    ODEOpenAngle(int _unum, double _dist_self_to_opp,
                 double _dist_self_to_opp_proj, double _dist_opp_proj,
                 double _open_angle) {
        unum = _unum;
