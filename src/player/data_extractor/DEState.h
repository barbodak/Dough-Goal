...
class DEBall {
  public:
    ...
    DEBall() {}
    DEBall(const BallObject &ball) : M_ball(&ball) {
        M_pos = ball.pos();
        M_rpos = ball.rpos();
    }
...
class DEPlayer {
  public:
    ...
    bool M_body_valid;
    DEPlayer(const AbstractPlayerObject *p, DEBall &ball) : M_player(p) {
        M_pos = p->pos();
        M_unum = p->unum();
        M_side = p->side();
...
class DEState {
  public:
    ...
    bool current_cycle;
    DEState(const WorldModel &wm) : M_wm(wm) {
        ...
        for (auto p : wm.allPlayers()) {
            ...
            dlog.addText(Logger::BLOCK,
                         "##side:%d, unum:%d is added to all_players",
                         p->side(), p->unum());
#endif
    ...
    void updateVectors(const WorldModel &wm) {
        ...
        for (auto p : M_all_players) {
#ifdef DESDebug
            dlog.addText(Logger::BLOCK, "--P side%d unum%d", p->side(),
                         p->unum());
#endif
            if (p->side() == ourSide()) {
#ifdef DESDebug
...
#ifdef DESDebug
                dlog.addText(Logger::BLOCK,
                             "####pushed to ourPlayers, teammates");
#endif
                if (p->unum() != -1) {
                    M_known_teammates[p->unum()] = p;
#ifdef DESDebug
                    dlog.addText(Logger::BLOCK,
                                 "#####add to kn tm %d %d, add %s", p->unum(),
                                 M_known_teammates[p->unum()]->unum(), p);
#endif
                }
            } else {
#ifdef DESDebug
...
#ifdef DESDebug
        dlog.addText(Logger::BLOCK, "----M_known_teammates--- %d",
                     M_known_teammates.size());
        for (auto p : M_known_teammates) {
            if (p != nullptr)
                dlog.addText(Logger::BLOCK, "## tm %d side %d %s", p->unum(),
                             p->side(), p);
        }
#endif
